...
import {SelfCenter} from "../component/SelfCenter";
...
class SiderDemo extends React.Component {
    ...
    componentWillMount() {
        let adUserId=localStorage.getItem('adUserId');
        if(adUserId===null)
            window.location.replace("http://"+localhost_frontend+"/?#/logIn");
        else
            this.setState({
                adUserId:parseInt(adUserId)
            })
    }
                ...
                <Layout className="site-layout">
                    ...
                    <Content style={{ margin: '0 16px' }}>
                        <div className="site-layout-background" style={{ padding: 24, minHeight: 360 }}>
                            <SelfCenter adUserId={this.state.adUserId}/>
                        </div>
                    </Content>
